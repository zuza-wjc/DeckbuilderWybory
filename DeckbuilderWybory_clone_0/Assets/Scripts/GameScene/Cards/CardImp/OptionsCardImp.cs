...
public class OptionsCardImp : MonoBehaviour
{
    ...
    public async void CardLibrary(string instanceId, string cardIdDropped, bool ignoreCost)
    {
        ...
        cardType = snapshot.Child("type").Exists ? snapshot.Child("type").Value.ToString() : throw new Exception("Branch type does not exist.");
            ...
            {
                budgetChange = true;
                ProcessBonusOptions(snapshot.Child("budget"), budgetBonusOptionsDictionary);
                ProcessOptions(snapshot.Child("budget"), budgetOptionsDictionary);
            }
        ...
        if (!(await cardUtilities.CheckBlockedCard(playerId)))
        {
            ...
            if (supportChange)
            {
                (playerBudget, ignoreCost, isBonusRegion, enemyId) = await SupportAction(budgetChange, playerBudget, cardIdDropped, ignoreCost, chosenRegion, isBonusRegion, cardType, supportOptionsDictionary, supportBonusOptionsDictionary, enemyId, budgetOptionsDictionary);
            }
